...
import { Send, Minus, ChevronRight } from 'lucide-react';
...
const TEASER_DISMISS_KEY = 'xiaomi-assistant-teaser-dismissed';

export function AiAssistant() {
  const [isOpen, setIsOpen] = useState(false);
  const [showTeaser, setShowTeaser] = useState(() => {
    if (typeof window === 'undefined') return false;
    return !localStorage.getItem(TEASER_DISMISS_KEY);
  });
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(dismissTeaser, 8000);
  ...
  return (
    <>
      {/* Botones flotantes: el contenedor no captura clics fuera de los botones */}
      <div
        className={`fixed z-[60] flex flex-col items-end gap-2 transition-all duration-300 origin-bottom-right pointer-events-none bottom-[max(1rem,env(safe-area-inset-bottom))] right-[max(0.75rem,env(safe-area-inset-right))] md:bottom-6 md:right-6 md:gap-3 ${
      ...
      >
        ...
        <a
          ...
          className="pointer-events-auto flex items-center justify-center w-11 h-11 md:w-[52px] md:h-[52px] bg-[#25D366] hover:bg-[#1ebe5d] text-white rounded-full shadow-lg md:shadow-xl hover:scale-105 transition-transform"
        >
        ...
        <div className="flex items-end gap-2 md:gap-3">
          {/* Burbuja: solo tablet/desktop; en móvil no bloquea la pantalla */}
          {showTeaser && (
            <button
              type="button"
              onClick={openChat}
              className="pointer-events-auto hidden md:flex items-center gap-2 max-w-[280px] bg-white px-3.5 py-2 rounded-2xl rounded-br-sm shadow-md border border-orange-100 text-left hover:scale-[1.02] transition-transform"
            >
          ...
          {/* Teaser corto solo móvil (primera visita, 8s) */}
          {showTeaser && (
            <button
              type="button"
              onClick={openChat}
              className="pointer-events-auto md:hidden flex items-center gap-1.5 max-w-[148px] bg-white/95 backdrop-blur-sm px-2.5 py-1.5 rounded-full shadow-md border border-orange-100 text-[11px] font-semibold text-gray-800 leading-tight"
            >
              <span className="w-1.5 h-1.5 bg-green-500 rounded-full shrink-0" />
              <span>¿Ayuda con tu Xiaomi?</span>
            </button>
          )}
          ...
          <button
            type="button"
            onClick={openChat}
            className="pointer-events-auto relative flex items-center justify-center w-12 h-12 md:w-[60px] md:h-[60px] shrink-0"
            aria-label="Abrir asistente Karol Garcia"
          >
            <div className="absolute inset-0 bg-orange-500 rounded-full animate-ping opacity-20 duration-[3000ms] hidden md:block" />
            <div className="w-12 h-12 md:w-[60px] md:h-[60px] bg-gradient-to-tr from-orange-500 to-orange-400 rounded-full flex items-center justify-center text-white font-bold text-base md:text-xl shadow-xl z-10 border-2 border-gray-900">
              KG
            </div>
          </button>
        </div>
      ...
      {/* Ventana del chat */}
      <div
        className={`fixed z-[70] flex flex-col overflow-hidden bg-[#fdfdfd] border border-gray-100 shadow-[0_10px_40px_rgba(0,0,0,0.2)] transition-all duration-300 origin-bottom-right inset-x-3 bottom-[max(0.75rem,env(safe-area-inset-bottom))] h-[min(100dvh-1.5rem,650px)] rounded-2xl sm:inset-x-auto sm:right-6 sm:bottom-6 sm:w-[390px] sm:h-[min(650px,calc(100dvh-3rem))] ${
